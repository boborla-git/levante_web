...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nuovaPassword === '' || $confermaPassword === '') {
        ...
        $sql = "SELECT id_utente, password_hash
                FROM aut_utenti
                WHERE id_utente = :id_utente
                LIMIT 1";

        $stmt = $pdo->prepare($sql);
        $stmt->execute(['id_utente' => (int)$_SESSION['utente_id']]);
        $utente = $stmt->fetch();
        ...
        if (!$utente) {
            ...
            if (!$obbligoCambioPassword) {
                ...
                } elseif (!password_verify($passwordAttuale, (string)$utente['password_hash'])) {
            ...
            if ($errore === '' && $verificaPasswordAttuale) {
                ...
                $stmtUpdate->execute([
                    ...
                    'id_utente' => (int)$_SESSION['utente_id'],
                ]);
