...
SELECT p.id_ruolo, r2.id_risorsa, 'view', 1
...
LEFT JOIN aut_ruoli_permessi x
       ON x.id_ruolo = p.id_ruolo
      AND x.id_risorsa = r2.id_risorsa
      AND x.permesso = 'view'
...
WHERE r2.codice_risorsa = 'pagina.report_assenze'
  ...
  AND p.id_ruolo IN (
      ...
      FROM (
          ...
          WHERE r1.codice_risorsa = 'pagina.approvazioni_assenze'
            AND p2.permesso = 'view'
            AND p2.consentito = 1
      ) ruoli_report
  );

INSERT INTO aut_ruoli_permessi (id_ruolo, id_risorsa, permesso, consentito)
SELECT p.id_ruolo, r2.id_risorsa, 'edit', 0
FROM aut_ruoli_permessi p
JOIN aut_risorse r1 ON r1.id_risorsa = p.id_risorsa
JOIN aut_risorse r2 ON r2.codice_risorsa = 'pagina.report_assenze'
LEFT JOIN aut_ruoli_permessi x
