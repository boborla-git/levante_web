...
?>
<div class="page-container"><div class="card card-wide"><div class="section-head"><div><h1>Permessi tutelati</h1><div class="meta">Situazione personale dei permessi abilitati da HR.</div></div></div><div class="info-box" style="margin-top:14px;">In questa fase le registrazioni dei permessi tutelati vengono inserite dal responsabile o da HR. Qui puoi consultare utilizzi e disponibilità residue.</div></div>
...
<?php else:?>
<div class="card card-wide"><div class="section-head"><h2>Legge 104 - <?=sprintf('%02d/%04d',$mese,$anno)?></h2><form method="get" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><label class="meta" for="mese104">Mese:</label><select id="mese104" name="mese" style="width:90px"><?php for($m=1;$m<=12;$m++):?><option value="<?=$m?>" <?=$m===$mese?'selected':''?>><?=sprintf('%02d',$m)?></option><?php endfor;?></select><label class="meta" for="anno104">Anno:</label><input id="anno104" type="number" name="anno" value="<?=$anno?>" min="2020" max="2100" style="width:100px"><button class="btn" type="submit">Vai</button></form></div>
